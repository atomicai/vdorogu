...
def test_get_callables(
    ...
    assert mock_modules.call_count == 1, (
        f'should be exectly one call for `ReportInFilesystem.get_modules`, ' f'but got {mock_modules.call_count} calls'
    )
    mock_std_params.assert_called()
    assert mock_std_params.call_count == 1, (
        f'should be exectly one call for `ReportInFilesystem.get_standard_parameters`, '
        f'but got {mock_std_params.call_count} calls'
    )
    mock_dag_constants.assert_called()
    assert mock_dag_constants.call_count == 1, (
        f'should be exectly one call for `Report._get_dag_constants`, ' f'but got {mock_dag_constants.call_count} calls'
    )
...
def test_get_callable_out(
    ...
    assert etalon_dictionary_count == len(result_dictionaries), (
        f'should be exactly {etalon_dictionary_count} out dictionaries, '
        f'but got {len(result_dictionaries)} dictionaries'
    )
    necessary_fields = {
        'report_name',
        'name',
