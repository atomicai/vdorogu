...
def test_all_reports(
    ...
    assert (
        mock_glob.call_count == 1
    ), f"should be exectly one call for `pathlib.Path.glob`, but got {mock_glob.call_count} calls"
    mock_is_dir.assert_called()
    assert mock_is_dir.call_count == len(
        side_effect
    ), f"should be exectly {len(side_effect)} calls for `pathlib.Path.is_dir`, but got {mock_glob.call_count} calls"
...
def test_find_report_path(name: str, report_paths: List[Path], etalon_path: Path) -> None:
    def find_report_path_call() -> Path:
        ...
        ), f"should be exectly two calls for `pathlib.Path.glob`, but got {mock_glob.call_count} calls"
        return result_path
...
def test_get_modules(
    ...
    mock_is_dir.assert_called()
    assert (
        mock_is_dir.call_count == 1
    ), f"should be exectly one call for `pathlib.Path.is_dir`, but got {mock_glob.call_count} calls"
...
def test_get_parameters(
    ...
    assert mock_report_path.call_count == 1, (
        "should be exectly one call for `ReportInFilesystem.find_report_path`, "
        f"but got {mock_report_path.call_count} calls"
    )
    mock_is_dir.assert_called()
    assert (
        mock_is_dir.call_count == 1
    ), f"should be exectly one call for `pathlib.Path.is_dir`, but got {mock_is_dir.call_count} calls"
    if is_dir:
        mock_open.assert_called()
        assert (
            mock_open.call_count == 1
        ), f"should be exectly one call for `builtins.open`, but got {mock_open.call_count} calls"
    else:
        mock_read_text.assert_called()
        assert (
            mock_read_text.call_count == 1
        ), f"should be exectly one call for `pathlib.Path.read_text`, but got {mock_read_text.call_count} calls"
