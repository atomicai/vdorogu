...
def test_ddl_path(
    ...
    def ddl_path_call() -> Path:
        ...
        assert mock_glob.call_count == 3, (
            'should be exectly 3 calls for `pathlib.Path.glob`, ' f'but got {mock_glob.call_count} calls'
        )
        return test_result
...
def test_metadata_path(
    ...
    def metadata_path_call() -> Path:
        ...
        mock_glob.assert_called()
        assert mock_glob.call_count >= 3, (
            'should be more then 3 calls for `pathlib.Path.glob`, ' f'but got {mock_glob.call_count} calls'
        )
        if aggregator_paths:
