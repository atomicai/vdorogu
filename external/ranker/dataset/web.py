...
def t5_ptuning_concat(tokenizer, query, document, promt_lenght=[3, 3, 3]):
    return np.concatenate(
        [[-1] * (promt_lenght[0] - 1) + [-2], query, [-2] * promt_lenght[1], document, [-2] * promt_lenght[2]]
    )
...
class FlatDataset(Dataset):
    def __init__(
        self, queries, documents, labels, qids, tokenizer, max_len=128, mapper=None, token_concat_flavour=bert_concat
    ):
        self.queries = queries
        self.documents = documents
        self.labels = labels
...
class QueryDatasetDSSM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for query, document, label, qid in zip(
            ...
            document_ids.append(
                np.concatenate(
                    [[self.tokenizer.bos_token_id], document[: self.max_len - 2], [self.tokenizer.eos_token_id]]
                )
            )
            labels.append(label)
            qids.append(qid)
        ...
        for bucket_idx, indices in sorted(self.batch_map.items(), key=lambda x: x[0]):
            ...
            for group in [indices[i : (i + batch_size)] for i in range(0, len(indices), batch_size)]:
                if len(group) == batch_size:
                    ...
                    resid_list += group
        for group in [
            resid_list[i : (i + self.batch_sizes[-1])] for i in range(0, len(resid_list), self.batch_sizes[-1])
        ]:
            batch_list.append(group)
        return batch_list
...
class BucketBatchSyncSampler(Sampler):
    # want inputs to be an array
    def __init__(
        self, length, bins=[32, 64, 128, 196, 256], batch_sizes=[128, 64, 32, 20, 16, 8], shuffle=True, world_size=4
    ):
        self.batch_sizes = batch_sizes
        self.bins = bins
        self.batch_map = defaultdict(list)
