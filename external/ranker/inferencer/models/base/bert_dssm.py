...
from external.ranker.nn_component.model.nlp.bert_dssm import Bert, BertDSSM, BertSentenceEmbDSSM
...
class BertContainer(Container):
    ...
    def prepare_data(self, query, document):
        ...
        document = self.tokenizer.encode(
            " ".join(document.lower().split()[: self.document_maxlen]), add_special_tokens=False
        )
