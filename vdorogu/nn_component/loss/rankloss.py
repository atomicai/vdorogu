...
class PairwiseLogisticLoss(nn.Module):
    ...
    def forward(self, pairwise_logits):
        ...
        return torch.relu(-pairwise_logits) + torch.log1p(
...
class PairwiseSoftZeroOneLoss(nn.Module):
    ...
    def forward(self, pairwise_logits):
        return torch.where(pairwise_logits > 0, 1. - torch.sigmoid(pairwise_logits), torch.sigmoid(-pairwise_logits))
...
class RankingLossListWise(nn.Module):
    def __init__(self, pairwise_loss = PairwiseLogisticLoss(),topn=50,smooth_fraction=0.,gain_fn=power_gain):
        super().__init__()
        self.topn = topn
    ...
    def forward(self, logits, labels):
        ...
        with torch.no_grad():
            ...
            discount = torch.where(ranks > self.topn, torch.zeros_like(ranks), 1. / torch.log1p(ranks))
            ...
            relative_discouts = torch.where(rank_diff > 0, torch.abs(
                1. / torch.log1p(rank_diff) - 1. / torch.log1p(rank_diff+1)), torch.zeros_like(rank_diff))
            ...
            discounted_gain = self.gain_fn(sorted_labels) * (1. / torch.log1p(ranks))
...
class RankingLossListWiseDistil(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        with torch.no_grad():
            ...
            #max_dcg
            ranks = (torch.arange(labels.shape[-1],dtype=labels.dtype,device=labels.device).view(1,-1) + 1).float()
            discounted_gain = (1. / torch.log1p(ranks)) * is_valid.float()
            discounted_gain = torch.where(ranks > self.topn, torch.zeros_like(labels), discounted_gain)
            max_dcg = torch.sum(discounted_gain,-1,keepdim=True)
            max_idcg = torch.where(max_dcg > 0, 1 / max_dcg, torch.zeros_like(max_dcg))
        ...
        loss = self.pairwise_loss((pairwise_logits_diff / self.temperature), (pairwise_labels_diff / self.temperature).sigmoid()) 
