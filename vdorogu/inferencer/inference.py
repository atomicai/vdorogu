...
class Inferencer:
    ...
    def debug_(self, dataset):
        ...
        self.prepare_container_()
        loader = DataLoader(dataset, batch_size=max(1, self.gpus),
...
def add_args():
    parser = argparse.ArgumentParser(
        description="Inferencer for all prod models", add_help=False)

    parser.add_argument('--bs', '--batch_size', type=int, default=128,
                               help='batch size for inference')

    parser.add_argument('--debug', dest='debug', action='store_true',
                               help='just print debug scores for model')
    ...
    parser.add_argument('--mode', type=str, default="scores",
                               help='variant of calculation (usually scores/embeddings)')

    parser.add_argument('--gpus', type=str, default='0',
                               help='number of gpus to compute, also accepts "all"')
                               
    parser.add_argument('--no_half', dest='half', action='store_false',
                               help='do not use fp16 on gpu')

    parser.add_argument('--storage_path', type=str, default=None,
                               help='path to model data storage, can be altered by --model_data_path')
    
    parser.add_argument('--model_data_path', type=str, default=None,
                               help='path to model data, usually can be automatically inferred from --storage_path')

    parser.add_argument("--model_params", dest="model_params", default={}, action=StoreDictKeyPair, nargs="+", metavar="KEY=VAL", help='model params')
