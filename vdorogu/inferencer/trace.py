...
from vdorogu.inferencer.inference import Inferencer, load_model
...
def get_args():
    ...
    parser.add_argument('--' + Container.DATA_PATH_NAME, type=str, default=None,
                               help='path to model data, usually can be automatically inferred from --storage_path')

    parser.add_argument('--output', type=str, default='model.onnx',
                               help='output fname')

    parser.add_argument('--quant', dest='quant', default=False, action='store_true',
                               help='save quant version of onnx model')
...
def main(hparams, model_params):
    Inferencer.prepare_model_params_(hparams, model_params, storage_path=hparams.storage_path, model_data_path=hparams.model_data_path)
    ...
    torch.onnx.export(to_trace, [batch], hparams.output,
                        opset_version=12,
                        verbose=False,
                        input_names=inp_names, output_names=out_names,
