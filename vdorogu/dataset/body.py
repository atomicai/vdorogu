...
class FlatDatasetDSSM(Dataset):
    def __init__(self, queries, documents, labels, qids, tokenizer, query_maxlen, body_maxlen, mapper=None, dssm_variant=True):
...
class QueryDatasetDSSM(Dataset):
    def __init__(
        self, queries, documents, labels, qids, tokenizer, max_query, query_maxlen, body_maxlen, mapper=None, priority_label=None
    ):
        '''
    ...
    def __getitem__(self, idx):
        ...
        for query, document, label in zip(self.queries[indices], self.documents[indices], self.labels[indices]):
            ...
            query_ids.append(
                np.concatenate([[self.tokenizer.bos_token_id], query[: self.query_maxlen - 2], [self.tokenizer.eos_token_id]])
            )
            document_ids.append(
                np.concatenate([[self.tokenizer.bos_token_id], document[: self.body_maxlen - 2], [self.tokenizer.eos_token_id]])
            )
            labels.append(label)
...
class MultizoneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for fields in zip(self.queries[indices], *[self.documents[field][indices] for field in self.fields], self.labels[indices]):
            ...
            last_zone_len = self.max_len - self.extra_tokens_num - len(query) - np.sum([len(zone) for zone in document_zones[:-1]])
...
class FlatMultizoneDataset(Dataset):
    ...
    def __init__(
        ...
        self.fields = fields
        self.mapper = mapper
        self.extra_tokens_num = len(self.token_concat_flavour(self.tokenizer, [], *[[] for i in range(len(fields))]).flatten())
        # print(self.dummy_token_id, self.extra_tokens_num)
...
class FlatPassageDatasetDSSM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = np.concatenate([[self.tokenizer.bos_token_id], document[: doc_maxlen - 2], [self.tokenizer.eos_token_id]])
