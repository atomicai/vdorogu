...
class RawTextDataset(IterableDataset):
    ...
    def __init__(self,
                filename, tokenizer, preprocessing,
                max_len=128, separator='\t', mapper=None,
                actual_fields = (0,1,2), max_len_list = None
        ):
        ...
        if mapper: self.mapper = lambda x: mapper.get(x, 3)
...
def bert_concat(tokenizer, query, document):
    return np.concatenate([
        [tokenizer.bos_token_id], query, [tokenizer.sep_token_id,
        tokenizer.sep_token_id], document, [tokenizer.eos_token_id]
        ])

def roberta_concat(tokenizer, query, document):
    return np.concatenate([
        [0], query, [2, 2], document, [2]])
...
def t5_concat(tokenizer, query, document):
    return np.concatenate([
        [tokenizer.bos_token_id], query, [tokenizer.eos_token_id,
        tokenizer.eos_token_id], document, [tokenizer.eos_token_id]
        ])
...
class QueryDataset(Dataset):
    ...
    """
    def __init__(self, queries, documents, labels, qids, tokenizer, max_query = 16,
    max_len = 128, mapper=None, priority_label=None, token_concat_flavour=bert_concat,
    label_count=1, dummy_label = -1000.):
    ...
    def __getitem__(self, idx):
        ...
        for query, document, label in zip(self.queries[indices], self.documents[indices], self.labels[indices]):
            ...
            query, document,_ = self.tokenizer.truncate_sequences(query, document,
                num_tokens_to_remove = len(query) + len(document) + self.extra_tokens_num - self.max_len )
...
class QueryDatasetWithNegativeSampling(Dataset):
    ...
    def __init__(self, queries, documents, labels, qids, tokenizer, max_query = 16, max_len = 128, mapper=None, positive_label_mask=None, p = 0.2, token_concat_flavour=bert_concat, label_count=1):
        self.token_concat_flavour = token_concat_flavour    
        ...
        self.max_len = max_len
        self.dummy_token_id = self.token_concat_flavour(self.tokenizer, [], []).flatten()
        self.dummy_label = [-1000.] * label_count
        self.mapper = mapper
    ...
    def __getitem__(self, idx):
        ...
        if size > 0:
            indices_neg = np.random.choice(np.arange(begidx + positive_label_count, endidx),
                                           size=size,
                                           replace = False)
...
class FlatDatasetWithEmb(Dataset):
    def __init__(self, queries, documents, labels, qids, tokenizer, query_emb, max_len = 128, mapper=None, token_concat_flavour=bert_concat):
        self.queries = queries
        self.documents = documents
        self.labels = labels
        self.qids = qids
...
class QueryDatasetDSSM(Dataset):
    ...
    def __init__(self, queries, documents, labels, qids, tokenizer, max_query = 16, max_len = 128,
    mapper=None, priority_label=None, valid_mode=False, dummy_label = -1000.):
        '''
    ...
    def __getitem__(self, idx):
        ...
        for query, document, label, qid in zip(self.queries[indices], self.documents[indices], self.labels[indices], self.qids[indices]):                    
            if self.mapper:
                query = np.array([self.mapper.get(i,3) for i in query], dtype=np.int32)
                document = np.array([self.mapper.get(i,3) for i in document], dtype=np.int32)

            query_ids.append(np.concatenate([[self.tokenizer.bos_token_id], query[:self.max_len-2], [self.tokenizer.eos_token_id]]))
            document_ids.append(np.concatenate([[self.tokenizer.bos_token_id], document[:self.max_len-2], [self.tokenizer.eos_token_id]]))
            labels.append(label)
            qids.append(qid)
...
class RankPairPermutationDatasetTrain(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        objects.append(
            np.concatenate(
                [_0, _q[:64], _2_2, _docs[0][:64], _2_2, _docs[1][:64], _2]
            )
        )
        ...
        objects.append(
            np.concatenate(
                [_0, _q[:64], _2_2, revert_doc[0][:64], _2_2, revert_doc[1][:64], _2]
            )
        )
...
class RankPairPermutationDatasetValid(Dataset):
    ...
    def __init__(
        self, queries, documents, labels, qids, 
        tokenizer, max_len=64, mapper=None,
        ):
    ...
    def no_collate(batch, pad_token_id = 1):
        ...
        input_queries_ids = pad8_sequence([
            torch.from_numpy(x) for x in inputs[0]],
...
class QueryDatasetMultitargetWithNegative(Dataset):
    ...
    def __init__(self, queries, documents, labels, qids, tokenizer, max_query = 16, 
                max_len = 128, mapper=None, priority_label=None, is_dssm=False,
                dummy_label=-1000., negative = 'default', min_length = 3, token_concat_flavour=bert_concat,
                use_emb=False):
        self.queries = queries
        self.documents = documents
        ...
        self.max_query = max_query
        self.max_len = max_len
        self.dummy_token_id = np.array([tokenizer.bos_token_id,tokenizer.eos_token_id,tokenizer.eos_token_id,tokenizer.eos_token_id])
        self.dummy_label = dummy_label
        self.mapper = mapper
        self.priority_label = priority_label
    ...
    def __getitem__(self, idx):
        ...
        for query, document, label in zip(quers, self.documents[indices], self.labels[indices]):
            ...
            if not self.is_dssm:
                query, document, _ = self.tokenizer.truncate_sequences(query, document,
                    num_tokens_to_remove = len(query) + len(document) + 4 - self.max_len )
        ...
        if not self.priority_label:
            ...
            for _ in range(negatives_count):
                ...
                if not self.is_dssm:
                    ...
                    document_list.append(
                        np.concatenate([[self.tokenizer.bos_token_id], self.documents[neg_idx][:self.max_len-2], [self.tokenizer.eos_token_id]])
                    )
        ...
        labels = np.concatenate(
            [
                np.array(labels),
                np.array(positive_labels).reshape(-1, 1)
            ], axis=1
        )
        if not self.is_dssm:
            return input_ids, labels
...
class QueryHardNegativeDataset(Dataset):
    def __init__(self, queries, documents, labels, qids,
                 negative_queries, negative_documents,
                 negative_labels, negative_qids,
                 tokenizer, max_query = 16, max_negative_query = 8,
                 max_len = 128, is_dssm = False,
                 dummy_label = -1000., negative = 'free', min_length = 0, use_emb=False):
        self.dataset = QueryDatasetMultitargetWithNegative(
            queries=queries, documents=documents, labels=labels, qids=qids, tokenizer=tokenizer,
            max_query=max_query, max_len=max_len, is_dssm=is_dssm, negative=negative, dummy_label=dummy_label, min_length=min_length,
            use_emb=use_emb)
        
        self.negative_dataset = QueryDatasetMultitargetWithNegative(
            queries=negative_queries, documents=negative_documents, labels=negative_labels, qids=negative_qids, dummy_label=0.,
            tokenizer=tokenizer, max_query=max_negative_query, is_dssm=is_dssm, negative='free', min_length=min_length,
            use_emb=use_emb
        )
        self.is_dssm = is_dssm
...
def pad8_sequence(sequences, batch_first=True, padding_value=0., max_len=None):
    # assuming trailing dimensions and type of all the Tensors
    # in sequences are same and fetching those from sequences[0]
